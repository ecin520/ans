...
(
    `id`            INTEGER      NOT NULL AUTO_INCREMENT PRIMARY KEY,
    `role_name`     VARCHAR(300) NOT NULL UNIQUE,
...
(
    `id`                  INTEGER      NOT NULL AUTO_INCREMENT PRIMARY KEY,
    `permission_name`     VARCHAR(300) NOT NULL UNIQUE,
...
(
    ...
    `question_owner`    INTEGER       NOT NULL,
    `question_describe` VARCHAR(3600) NOT NULL,
    `question_explain`  VARCHAR(3600) NOT NULL,
    `item_a`            VARCHAR(30)   NOT NULL,
    `item_b`            VARCHAR(30)   NOT NULL,
    `item_c`            VARCHAR(30)   NOT NULL,
    `item_d`            VARCHAR(30)   NOT NULL,
    `item_ans`          VARCHAR(30)   NOT NULL,
...
INSERT `role`(`role_name`, `role_describe`)
VALUES ('admin', '管理员');
...
-- 插入权限
INSERT `permission` (`permission_name`, `permission_describe`)
VALUES ('普通', '答普通题的权限');
...
-- 通过用户id查询其权限
SELECT * FROM `permission` WHERE `id` IN (SELECT `pid` FROM `role_permission` WHERE `rid` IN (SELECT `rid` FROM `user_role` WHERE `uid` = 1));

SELECT * FROM `role`; DELETE FROM `role` WHERE `id` = 104;
...
INSERT `user` (`username`, `password`, `nickname`, `avatar_url`, `register_time`)
    VALUES ('ecin5201', 'qwer1234', '智学者', 'http://148.70.50.70/source/1572837893695.jpg', NOW());

SELECT * FROM `user`;

DELETE FROM `type` WHERE `id` = 104
